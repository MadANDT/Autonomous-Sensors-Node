...
/**
  * @brief LED sequence when entering Shutdown mode.
  * Green and red LEDs blink three times quickly.
  * @param None
  * @retval None
 */
void Shutdown_Entry_LEDSequence(void){
	for (int _ = 0; _ < 6; _++){
...
/**
  * @brief LED sequence when exiting Shutdown mode.
  * Green and red LEDs blink three times slowly.
  * @param None
  * @retval None
 */
void Shutdown_Exit_LEDSequence(void){
	for (int _ = 0; _ < 6; _++){
		HAL_GPIO_TogglePin(GPIOB,  GLED_Pin);
		HAL_GPIO_TogglePin(GPIOB,  RLED_Pin);
		HAL_Delay(200);
