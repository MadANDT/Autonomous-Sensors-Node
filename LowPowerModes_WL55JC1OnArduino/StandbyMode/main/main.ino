  ...
  ******************************************************************************
  * @file           : main.ino
  * @brief          : Main program body, enters Standby mode for a specified time -
...
/**
  * @brief Enter Standby mode for DELAY_S seconds,
  * wake up on RTC alarm interrupt.
  * @param delay_s: sleep duration offset in seconds
  * @retval None
 */
 void Enter_StandbyMode_WithRTCAlarm(uint16_t delay_s){
  Standby_Entry_LEDSequence();  // Indicate the entry in Standby mode with the LED sequence

  rtcAlarmFlag = false;
  
  RTC_Setup(delay_s);  

  // No need to suspend the SysTick Increment, as Standby is similar
  // to a full board-reset, meaning it must be the last function called.

  HAL_PWREx_EnterSTANDBYMode();
  /* ####----####----####----####
	 * MCU is asleep in Standby mode
	 * ####----####----####----####
	 */
}
...
void setup() {
  ...
  // Check if we resume from Standby sleep or normal reset
  ResumeFromStandby();
}
...
void loop(){
  ...
  Enter_StandbyMode_WithRTCAlarm(SLEEP_DURATION);  
